    ...
    namespace NRE {
        namespace Math {
            ...
            inline Plane<T>::Plane(Vector4D<K> const& n, Point4D<L> const& p) : normal(n), point(p / p.getW()) {
            }

            template <class T>
            template <class K, class L>
            inline Plane<T>::Plane(Vector3D<K> const& n, Point3D<L> const& p) : normal(n), point(p) {
            }

            template <class T>
            template <class K, class L, class N>
            Plane<T>::Plane(Point4D<K> const& p1, Point4D<L> const& p2, Point4D<N> const& p3) : Plane(Point3D<K>(p1 / p1.getW()), Point3D<L>(p2 / p2.getW()), Point3D<N>(p3 / p3.getW())) {
            }

            template <class T>
            template <class K, class L, class N>
            Plane<T>::Plane(Point3D<K> const& p1, Point3D<L> const& p2, Point3D<N> const& p3)  {
                computeNormalFrom3Point(p1, p2, p3);
            }

            template <class T>
            template <class K>
            Plane<T>::Plane(Plane<K> const& p) : normal(p.getNormal()), point(p.getPoint()) {
            }
            ...
            template <class T>
            template <class K, class L, class N>
            void Plane<T>::computeNormalFrom3Point(Point3D<K> const& p1, Point3D<L> const& p2, Point3D<N> const& p3) {
                auto u = p2 - p1;
                auto v = p3 - p1;
                setPoint(p1);
                setNormal(u ^ v);
